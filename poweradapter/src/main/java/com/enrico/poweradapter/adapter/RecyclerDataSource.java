...
public class RecyclerDataSource {
    ...
    private final Map<Integer, String> viewTypeToRendererKey = new HashMap<>();
    ...
    public RecyclerDataSource(Map<String, ItemRenderer<? extends RecyclerItem>> renderers) {
        ...
        for (Map.Entry<String, ItemRenderer<? extends RecyclerItem>> entry : renderers.entrySet()) {
            viewTypeToRendererKey.put(entry.getValue().layoutRes(), entry.getKey());
        }
    }
    ...
    /**
     * Allows us to set data without invoking DiffUtil which would throw an exception during testing
     */
    @VisibleForTesting
